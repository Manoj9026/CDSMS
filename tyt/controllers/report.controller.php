...
class reportController extends Controller{
      ...
      $this->data['issue'] = $this->_item->getFromQ($qu,array('0','1',$value->item_code));
        ...
        foreach ($this->data['issue'] as $k => $v) {
         ...
         $quary = 'SELECT  item_master.item_name,
                        ...
                        LEFT JOIN stations ON station_stock_master.station_id = stations.station_id
                        WHERE
    ...
    public function allitems(){
         /*get category details*/
           ...
           $where .= ' ORDER BY item_master.item_code ASC'; 
       ...
       /* public function itemdetails(){
            ...
            if(input::get('item_code')){
             ...
             $this->data['item'] = $this->_item->getFromQ($query,array($item_code,'1'))[0];
               ...
               $params = array('1', $item_code,$dates[0],$dates[1]);
                ...
                FROM
                bmd_iv_details
                LEFT JOIN bmd_iv_header ON bmd_iv_details.iv_header_id = bmd_iv_header.iv_header_id
        ...
        public function nill(){
          ...
          if($station_id=='2'){
            ...
            FROM
            item_master
            LEFT JOIN bmd_stock_master ON bmd_stock_master.item_code = item_master.item_code
            LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
            LEFT JOIN item_category ON item_master.item_cat_id = item_category.item_cat_id
            ...
            OR (SELECT COALESCE(Sum(bmd_iv_details.issue_qty)) FROM bmd_iv_details
                  ...
                  bmd_iv_details.active = ? AND
                  bmd_iv_details.issue_status = ? AND 
                  item_master.item_code=bmd_iv_details.item_code ) >=bmd_stock_master.qoh ORDER BY  item_master.item_code ASC';
                 ...
                 $query=   'SELECT station_stock_master.station_id,
                          ...
                          item_master.item_bin_no
                           FROM
                            ...
                            LEFT JOIN item_master ON station_stock_master.item_code = item_master.item_code
                            LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
                            LEFT JOIN item_category ON item_master.item_cat_id = item_category.item_cat_id
                          WHERE
                          station_stock_master.qoh < ? 
                          AND station_stock_master.station_id=?
                          ORDER BY  item_master.item_code ASC';
 ...
 public function pendingiv(){
          $query = 'SELECT bmd_iv_header.iv_no,
                          ...
                          FROM
                          bmd_iv_details
                          LEFT JOIN bmd_iv_header ON bmd_iv_header.iv_header_id = bmd_iv_details.iv_header_id
                          LEFT JOIN stations ON bmd_iv_header.iv_to = stations.station_id
                          WHERE
                          bmd_iv_details.active = ? AND
                          bmd_iv_details.issue_status = ?
          ...
          if($station_id=='2'){
            ...
            (SELECT COALESCE(Sum(bmd_iv_details.issue_qty),0) FROM bmd_iv_details
                  ...
                  $query = 'SELECT   station_stock_master.item_code,
                            ...
                            station_stock_master.station_id = ? ORDER BY item_master.item_code ASC';
  ...
  public function expire(){
         ...
         $query =  'SELECT
            ...
            LEFT JOIN bmd_crv_header ON bmd_crv_details.crv_header_id = bmd_crv_header.crv_id
            LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
            LEFT JOIN bmd_stock_master ON bmd_crv_details.item_code = bmd_stock_master.item_code
           ...
           $query =  'SELECT station_rv_header.rv_no,
                      ...
                      LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
                      LEFT JOIN station_stock_master ON station_rv_details.item_code = station_stock_master.item_code
                      WHERE
              ...
              $qcrv = 'SELECT station_rv_header.rv_no,
               ...
               $params = array('1', $item_code,$dates[0],$dates[1],$this->data['admin']->station_id);
                ...
                $qiv =  'SELECT user_master.`name`,
                        ...
                        FROM
                        user_master
                        LEFT JOIN station_iv_details ON station_iv_details.latest_user_tbl_id = user_master.user_id
                        LEFT JOIN station_iv_header ON station_iv_details.iv_header_id = station_iv_header.iv_id
                        WHERE
...
public function allitems_station(){
        if(input::get('item_cat_id')!=''){
          ...
          FROM
          item_category
          LEFT JOIN item_master ON item_master.item_cat_id = item_category.item_cat_id
          LEFT JOIN station_stock_master ON station_stock_master.item_code = item_master.item_code
          WHERE
          station_stock_master.station_id = ? AND
          station_stock_master.active = ? group by  item_category.item_cat_id'; 
