...
class bmdivController extends Controller{
	...
	public function index(){
       ...
       $query = 'SELECT bmd_iv_header.*, user_master.name, user_master.service_no, user_master.user_rank, stations.station_name, item_category.cat_name FROM bmd_iv_header 
        ...
        WHERE  bmd_iv_header.iv_type!=? AND bmd_iv_header.active = ?  ORDER BY bmd_iv_header.iv_header_id DESC';

        $pagination = new paginator($query,array('8','1') );
	...
	public function newiv(){
     ...
     if(input::exists()){
        ...
        $this->data['category'] = $this->_category->getCategory(array(array('active','=','1'),array('item_cat_id','!=','8')),array('cat_name'=>'ASC'));
    ...
    public function instnewiv(){
            $this->checkModuleAccess([2,4]);
     if(input::exists()){
            if(token::check(input::get('token'))){
                $validate = new validate();
                 if(input::get('iv_header_id')){
                     $bmdIv = array('name'=>'Referance','required'=>TRUE,'min'=>'3'); }else{ 
                         $bmdIv =array('name'=>'Referance','required'=>TRUE,'min'=>'3','unique'=>'bmd_iv_header');}
                $validate->check($_POST, array(
                    'iv_type'=>array('name'=>'Default Item Type','required'=>TRUE,'min'=>'1'),
                    'iv_no'=>$bmdID,
                    'iv_date'=>array('name'=>'IV Date','required'=>TRUE,'min'=>'3','date'=>'Y-m-d'),
                        ...
                        try {
                            $this->model->updateIv(array('iv_type'=> escape(input::get('iv_type')),
                            	...
                            	'latest_user_tbl_id'=> $this->data['admin']->user_id,
                              	...
                              	'active'=>'1'),array('iv_header_id','=', escape(input::get('iv_header_id'))));    
                            session::flash('success', 'Category updated');
                            router::redirect(HTTP_PATH.'bmdiv/instindex');
                            exit();
                        } catch (Exception $ex) {
                            session::flash('error', $ex->getMessage());
                            }
                        ...
                        try {
                            $this->model->createIv(array('iv_type'=> escape(input::get('iv_type')),
                                ...
                                'authority'=>escape(input::get('authority')),
                              	...
                              	'system_dt'=>getDT(),
                              	'active'=>'1'));
                            session::flash('success', 'New Iv created');
                            router::redirect(HTTP_PATH.'bmdiv/instindex');
                            exit();
                        } catch (Exception $ex) {
                            session::flash('error',$ex->getMessage());
      ...
      if(isset($this->getParams()[0])){
        ...
        if(input::exists()){
            if(token::check(input::get('token'))){
             ...
             'batch_no'=>array('name'=>'Batch Number','required'=>TRUE,'min'=>'4','max'=>'25'),
  ...
  public function instivdetails(){
     ...
     if(input::get('del')){
       if(parent::checkPermition(2)){ 
           try {
              ...
              $this->model->updateIvDetails(array('active'=>'0'),array('bmd_iv_detail_id','=', clearInt(input::get('del'))));
               session::flash('success', 'Item details deleted');
               router::redirect(HTTP_PATH.'bmdiv/instivdetails/'.$this->getParams()[0]);
               exit();
           } catch (Exception $ex) {
               session::flash('error', 'Item details not deleted!');
               router::redirect(HTTP_PATH.'bmdiv/instivdetails/'.$this->getParams()[0]);
             ...
             'iv_header_id'=>array('name'=>'IV header id','min'=>'1')
                ...
                if($validate->passed()){
                    try {
                     ...
                     //     session::flash('success', 'Iitem added to IV');
                      //    router::redirect(HTTP_PATH.'bmdiv/instivdetails/'.clearInt(input::get('iv_header_id')));
                      //    exit();
                      } catch (Exception $exc) {
                         session::flash('error', $ex->getMessage());
                      }                                        
    ...
    public function upStock($code,$units,$type){
     ...
     $current = $this->model->getCurrentQuantity(array('item_code','=',$code))[0]->qoh;
       switch ($type) {
       case 'IN':          $newStoc = $current+$units;          break;       
       default:            $newStoc = $current-$units;          break;
     }
       ...
       $this->data['tr'] ='';
        if($this->data['re']){
          foreach ($this->data['re'] as $key => $value) {
            $i++;
                $this->data['tr'] .='<tr>
                                    <td width="10mm" >'.$value->item_bin_no.'</td>                                        
       ...
       $this->data['tr'] ='';
        if($this->data['re']){
          foreach ($this->data['re'] as $key => $value) {
           ...
           $this->data['html'] = '<table border="0" width="100%"  ><tbody >
                                <tr>
                                    <td width="80mm" ><br/><br/><br/><br/><br/>'.$this->data['header'][0]->iv_no.'</td>
                                    <td width="5mm"></td>
                                    <td width="65mm">Ledger section
                                    <br/>Base Medical Depot
                                    <br/>Directorate of Army Medical Procument Sevice 
                                    <br/>Werahera
                                    <br/>Boralesgamuwa</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td width="80mm">See the Distribution</td>
                                    <td width="5mm"></td>
                                    <td width="65mm">'.date("Y F j" ,strtotime($this->data['header'][0]->iv_date)).'</td>
                                </tr>
                                <tr><td  width="80mm" > </td>
                                  <td  colspan="2" > </td> 
                                 </tr> <tr>
                                ...
                                <tr><td  width="80mm" ><u><b>Issuance of Dental Surgical Equipment</b></u></td>
                                  <td  colspan="2" > </td> 
                                 </tr>  
                                 <tr>
                                    <td width="80mm"></td>
                                 ...
                                 <tr><td  colspan="3">
                                 1.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;I respectfully propose that the following dental institutes submit an application for the year '.date("Y" ,strtotime($this->data['header'][0]->iv_date)).' and supply the following dental surgical tools to the appropriate operating theaters: 
                                </td></tr>
                                 
                                <tr>
                                 <td height="5mm" width="20mm" ></td>
                                  <td  width="130mm" > </td>   
                                 </tr>  
                                 <tr>
                                 <td height="5mm" width="20mm" ></td>
                                  <td  width="130mm" ><u><b>'.$this->data['station'][0]->station_name.'</b></u></td>   
                                 </tr>                                  
                                  </tbody>
                                  </table>
                                  <br/><table border="0" width="100%" ><tbody>'.$this->data['tr'];
          ...
          $this->data['html'] .= '<br/><br/><br/><br/><table border="0" width="100%" ><tbody>
                                <tr>
                                    <td width="80mm"></td>
                                    <td width="5mm"></td>
                                    <td width="65mm">'.$this->data['header'][0]->authority.'<br/>
                                    Officer Commanding - Ledger Division</td>
                                </tr>
                                <tr>
                                    <td width="80mm" height="10mm">Distribution / Activation:</td>
                                    <td width="5mm"></td>
                                    <td width="65mm"></td>
                                </tr>
                                <tr>
                                    <td width="40mm" height="10mm">Dte of ADS <br/> DBMD - BMD</td>
                                    <td width="5mm"> - </td>
                                    <td width="105mm">Make arrangements to include and issue the relevant surgical instruments in the Health 312 records.</td>
                                </tr>
                                <tr>
                                    <td width="40mm">'.$this->data['station'][0]->station_name.'</td>
                                    <td width="5mm"> - </td>
                                    <td width="105mm">Make Arrangements to take the relevant dental equipment before the '.date("Y").'.12.30.</td>
                                </tr>
