...
class ivController extends Controller{
	...
	public function index(){
       ...
       $query = 'SELECT station_iv_header.*, user_master.name, user_master.service_no, user_master.user_rank,  item_category.cat_name, stations.station_name FROM station_iv_header
        ...
        LEFT JOIN stations ON  station_iv_header.iv_to=stations.station_id
        WHERE station_iv_header.active = ? AND station_iv_header.station_id=?  
	...
	public function newiv(){
     ...
     $station = new stations();
     $this->data['iv_loc'] = $station->getStation(array(array('active','=','1'),array('station_id','>',2)));
  ...
  public function ivdetails(){
    ...
    return  $this->model->addToStationStock(array('station_id'=>$this->data['admin']->station_id,
      ...
      /*update the curant quantity with new araivals*/
