...
class rvController extends Controller {
    ...
    public function index(){        
        ...
        $pagination = new paginator('SELECT station_rv_header.*,  user_master.name, user_master.service_no,user_master.user_rank 
          ...
          LEFT JOIN user_master ON station_rv_header.latest_user_tbl_id = user_master.user_id
    ...
    public function rvfrom(){
        ...
        WHERE bmd_iv_header.active = ? and iv_to=? ORDER BY bmd_iv_header.iv_header_id DESC';
     ...
     //  print_r($this->data['details']);
        ...
        public function rvstations(){
        
        $query = 'SELECT station_iv_header.*, 
            user_master.name, user_master.service_no, user_master.user_rank,  item_category.cat_name, stations.station_name 
            FROM station_iv_header
        ...
        LEFT JOIN stations ON  station_iv_header.station_id=stations.station_id        
        WHERE station_iv_header.active = ? AND station_iv_header.iv_to=?        
        ORDER BY station_iv_header.iv_id DESC';
            ...
            if(token::check(input::get('token'))){
                ...
                $validate->check($_POST, array( 
                 'item_code'=>array('name'=>'Item name','required'=>TRUE,'min'=>'1'),  
                  ...
                  'batch_no'=>array('name'=>'Batch Number','required'=>FALSE,'min'=>'1','max'=>'25'),
                  'expire_date'=>array('name'=>'Expire Date','required'=>FALSE,'date'=>'Y-m-d','future'=>TRUE)
                  ));                
                if($validate->passed()){
                    try {
      ...
      if(isset($this->getParams()[0])){
        ...
        /*get current added crv items details*/
                      $sql =  'SELECT station_rv_details.tbl_id,
                                ...
                                FROM
                                station_rv_details
                                LEFT JOIN item_master ON station_rv_details.item_code = item_master.item_code
                                LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
                                LEFT JOIN user_master ON station_rv_details.latest_user_tbl_id = user_master.user_id
    ...
    public function viewcrv(){
      ...
      if(isset($this->getParams()[0])){
        ...
        FROM
        bmd_crv_details
        LEFT JOIN item_master ON bmd_crv_details.item_code = item_master.item_code
        LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
        LEFT JOIN user_master ON bmd_crv_details.latest_user_tbl_id = user_master.user_id
        WHERE
    ...
    public function frombmddetails(){
        
        $this->data['rv_created'] = false;
        $bmdiv = new bmdivs();
        if(isset($this->getParams()[0])){ 
             
            $this->data['ivheader'] =  $bmdiv->getIv(array(array('iv_header_id','=', escape($this->getParams()[0])),
                array('active','=','1'),
                array('iv_to','=',$this->data['admin']->station_id))); 
            
            //check alrady create rv if reate redirect to add item to it 
             if(!isset($this->getParams()[1])){
            $CratedRv = $this->model->getRv(array(array('refaral_bmd','=', escape($this->getParams()[0])),
                 array('station_id','=',$this->data['admin']->station_id)));
            
            if(count($CratedRv)>0){
              //  echo $CratedRv[0]->rv_id;
                router::redirect(HTTP_PATH.'rv/frombmddetails/'.escape($this->getParams()[0]).'/'.$CratedRv[0]->rv_id.'/');
            //    exit();
            }
             }
        ...
        if(count($this->data['ivheader'])==0){
            router::redirect(HTTP_PATH.'rv/rvfrom');
            exit();
        }        
            $this->_category =  new categorys();
            $this->data['category'] = $this->_category->getCategory(array('active','=','1'),array('cat_name'=>'ASC'));       
      ...
      //       $this->data['station'] = $supplier->getStation(array('station_id','=',$this->data['ivheader'][0]->iv_to)); 
         ...
         if(isset($this->getParams()[1])){
          $this->data['header'] =  $this->model->getRv(array(array('rv_id','=', escape($this->getParams()[1])),array('station_id','=',$this->data['admin']->station_id)));

          if(count($this->data['header'])==0){
            router::redirect(HTTP_PATH.'rv/rvfrom');
            exit();
          }else{
              $this->data['rv_created'] = true;
                foreach ($this->data['header'][0] as $key => $value) {
                 if($key=='rv_id'){ $rv_header_id =$value; input::put('rv_header_id',$value); continue;}
                input::put($key, $value);
                }               
                ...
                if($validate->passed()){ 
                      ...
                      try {
                        $id =  $this->model->createRv(array(
                              ...
                              'iv_date'=> escape(input::get('iv_date')),
                              'iv_from'=> (escape(input::get('iv_from'))),
                              'refaral_bmd'=>(escape(input::get('iv_header_id'))),
                              'station_id'=>$this->data['admin']->station_id,
                              'latest_user_tbl_id'=> $this->data['admin']->user_id,
                              'latest_ip'=> $this->ip,
                              'system_date'=>date('Y-m-d'),
                        ...
                        //update crete rv based on iv issued by bmd
                        $bmdiv->updateIv(array('added_to_stock'=>'1'),array('iv_header_id','=', input::get('iv_header_id')));
                        
                          session::flash('success', 'New RV header created');
                          router::redirect(HTTP_PATH.'rv/frombmddetails/'.clearInt(input::get('iv_header_id')).'/'.$id);
                          exit();
                      } catch (Exception $exc) {
                         session::flash('error', $ex->getMessage());
                      }                                        
                ...
                if($validate->passed()){
                    try {
                          $this->model->createRvDetails(array(
                              ...
                              'batch_no'=> escape(input::get('batch_no')),
                              'expire_date'=> escape(input::get('expire_date')),
                              'referal_id'=> escape(input::get('bmd_iv_detail_id')),
                              'latest_user_tbl_id'=> $this->data['admin']->user_id,
                              'station_id'=>$this->data['admin']->station_id,
                              'latest_ip'=> $this->ip,
                              'system_date'=>date('Y-m-d'),
                              'system_dt'=>getDT(),
                              'active'=>'1'));
                          // bmdiv details table update stock as receid
                        $bmdiv->updateIvDetails(array('added_to_stock'=>1),array('bmd_iv_detail_id','=', input::get('bmd_iv_detail_id')));
                          
                          $this->upStock(clearInt(input::get('item_code')),clearInt(input::get('receive_qty')),'IN');
                          session::flash('success', 'New RV item added and stock update');
                           router::redirect(HTTP_PATH.'rv/frombmddetails/'.clearInt(input::get('iv_header')).'/'.clearInt(input::get('rv_header_id')));
                         exit(); 
                      } catch (Exception $exc) {
                         session::flash('error', $ex->getMessage());
                      }                     
    ...
    $this->data['details'] =  $this->model->getFromQ($query,array('1',clearInt($this->getParams()[0])));
     ...
     public function rvstationsdetails(){
        
        $this->data['rv_created'] = false;
        $ivs = new ivs();
        if(isset($this->getParams()[0])){ 
             
            $this->data['ivheader'] =  $ivs->getIv(array(array('iv_id','=', escape($this->getParams()[0])),
                array('active','=','1'),
                array('iv_to','=',$this->data['admin']->station_id))); 
            
            //check alrady create rv if reate redirect to add item to it 
             if(!isset($this->getParams()[1])){
            $CratedRv = $this->model->getRv(array(array('refer_st_id','=', escape($this->getParams()[0])),
                 array('station_id','=',$this->data['admin']->station_id)));
            
            if(count($CratedRv)>0){
              //  echo $CratedRv[0]->rv_id;
                router::redirect(HTTP_PATH.'rv/rvstationsdetails/'.escape($this->getParams()[0]).'/'.$CratedRv[0]->rv_id.'/');
            //    exit();
            }
             }
        ...
        if(count($this->data['ivheader'])==0){
            ...
            $this->data['category'] = $this->_category->getCategory(array('active','=','1'),array('cat_name'=>'ASC'));       
            $supplier = new stations();
            $this->data['station'] = $supplier->getStation(array('station_id','=',$this->data['ivheader'][0]->station_id)); 
        } else{
            router::redirect(HTTP_PATH.'rv/rvstations');
            exit();
        }
         ...
         if(input::exists()){
            if(token::check(input::get('token'))){                
                ...
                if(input::get('iv_id')!=''){                
                if(input::get('rv_id')){
                    $rv = array('name'=>'RV No','required'=>TRUE,'min'=>'1'); }else{ 
                    $rv =array('name'=>'RV No','required'=>TRUE,'min'=>'1','unique'=>'station_rv_header');}
                $validate->check($_POST, array(
                 'rv_type_id'=>array('name'=>'Recieve Category','required'=>TRUE,'min'=>'1'), 
                    'rv_no'=>$rv, 
                    'rv_date'=>array('name'=>'RV Date','required'=>TRUE,'date'=>'Y-m-d'),                     
                ...
                if($validate->passed()){ 
                      ...
                      try {
                        $id =  $this->model->createRv(array(
                              ...
                              'iv_date'=> escape(input::get('iv_date')),
                              'iv_from'=> (escape(input::get('iv_from'))),
                              'rafaral_station'=>(escape(input::get('iv_id'))),
                              'station_id'=>$this->data['admin']->station_id,
                              'latest_user_tbl_id'=> $this->data['admin']->user_id,
                              'latest_ip'=> $this->ip,
                              'system_date'=>date('Y-m-d'),
                        ...
                        //update crete rv based on iv issued by bmd
                        $ivs->updateIv(array('added_to_stock'=>'1'),array('iv_id','=', input::get('iv_id')));
                        
                          session::flash('success', 'New RV header created');
                          router::redirect(HTTP_PATH.'rv/rvstationsdetails/'.clearInt(input::get('iv_id')).'/'.$id);
                          exit();
                      } catch (Exception $exc) {
                         session::flash('error', $ex->getMessage());
                      }                                        
                ...
                if($validate->passed()){
                    try {
                          $this->model->createRvDetails(array(
                              ...
                              'batch_no'=> escape(input::get('batch_no')),
                              'expire_date'=> escape(input::get('expire_date')),
                              'referal_id'=> escape(input::get('tbl_id')),
                              'latest_user_tbl_id'=> $this->data['admin']->user_id,
                              'station_id'=>$this->data['admin']->station_id,
                              'latest_ip'=> $this->ip,
                              'system_date'=>date('Y-m-d'),
                              ...
                              'active'=>'1'));
                          // bmdiv details table update stock as receid
                          $ivs->updateIvDetails(array('added_to_stock'=>1),array('tbl_id','=', input::get('tbl_id')));
                          
                          $this->upStock(clearInt(input::get('item_code')),clearInt(input::get('receive_qty')),'IN');
                          session::flash('success', 'New RV item added and stock update');
                           router::redirect(HTTP_PATH.'rv/rvstationsdetails/'.clearInt(input::get('iv_header')).'/'.clearInt(input::get('rv_header_id')));
                         exit(); 
                      } catch (Exception $exc) {
                         session::flash('error', $ex->getMessage());
                      }                     
