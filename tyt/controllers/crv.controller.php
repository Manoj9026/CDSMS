...
class crvController extends Controller {
            ...
            if(token::check(input::get('token'))){
                ...
                $validate->check($_POST, array(
                 ...
                 'crv_type_id'=>array('name'=>'Recieve Type','required'=>TRUE,'min'=>'1'),  
                    ...
                    'remarks'=>array('name'=>'Remarks','max'=>400),
                ));                
                if($validate->passed()){
                  if(input::get('crv_id')){
                      //update crv
            ...
            if(token::check(input::get('token'))){
                ...
                $validate->check($_POST, array(
                 ...
                 'item_code'=>array('name'=>'Item name','required'=>TRUE,'min'=>'1'),  
                  ...
                  'remarks_details'=>array('name'=>'Remarks','max'=>'250')
                  ));                
                if($validate->passed()){
                    try {
                          $this->model->createCrvDetails(array(
                            'item_cat_id'=>clearInt(input::get('item_cat_id')),
                              ...
                              'qty'=> clearInt(input::get('qty')),
                              'abl_stock'=> clearInt(input::get('qty')),
    ...
    public function upStock($code,$units,$type){
      /*get the current quantity*/
         $this->checkModuleAccess([1,4]);
         $abl = $this->model->getCurrentQuantity(array('item_code','=',$code));
         if(count($abl)>0){
            $current = $abl[0]->qoh;
         }else{ 
            $this->model->addCurrentQuantity(array('item_code'=>$code));
            $current=0;
         }
       //  $this->model->getCurrentQuantity(array('item_code','=',$code))[0]->qoh;
  ...
  if(isset($this->getParams()[0])){
        ...
        $this->data['re'] = $this->_crv->getFromQ($query,array($this->data['header'][0]->crv_id,'1'));
         ...
         $this->data['html'] = '<table border="0" width="100%" ><tbody>
                                <tr>
                                  <td width="35mm"></td>
                                  <td height="65" width="260" ></td>
                                  <td  width="287" >'.$result['supplier'][0]->supp_name.'</td>
                                  <td  width="90" ></td>
                                  <td  width="200" >BASE MEDICAL DEPORT<br/>'.$result['header'][0]->crv_no.'<br/>'.$result['header'][0]->crv_date.'</td>
                                 </tr>
                                  <tr>
                                  <td width="35mm"></td>
            ...
            $cat = ($value->item_cat_id==8)?'Acce':'Cons';
             $this->data['html'] .='<tr>
              ...
              while($i<12){
                ...
                $y=FALSE;
                $this->data['html'] .='<tr height="20" ><td></td><td>'.$x.'</td><td>'.$x.'</td><td>'.$x.'</td><td >'.$x.'</td><td style="text-align:right" >'.$x.'</td><td style="text-align:right" >'.$x.'</td><td style="text-align:right">'.$x.'</td></tr>';
                  }
                }          
              }
        }
