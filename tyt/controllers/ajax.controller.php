    ...
    /* get items with available quantity for issue and reciev*/ 
      ...
      if(isset($this->getParams()[0])){ 
        $quary = 'SELECT item_master.item_code,
                ...
                FROM item_master 
                LEFT JOIN bmd_stock_master ON item_master.item_code = bmd_stock_master.item_code
...
public function item_stock(){   
    ...
    public function supplier_search(){
      ...
      $quary =  'SELECT supplier_master.*, user_master.name, user_master.service_no, user_master.user_rank FROM supplier_master 
          LEFT JOIN user_master ON supplier_master.latest_user_tbl_id = user_master.user_id  
 ...
 return 'api';
    ...
    $this->_item = new items();
      ...
      $quary =  'SELECT item_master.*, item_category.cat_name, user_master.name, user_master.service_no, user_master.user_rank 
          FROM item_master 
          LEFT JOIN user_master ON item_master.latest_user_tbl_id = user_master.user_id 
          LEFT JOIN item_category ON item_master.item_cat_id = item_category.item_cat_id
          WHERE item_master.item_name LIKE ? OR item_master.item_bin_no LIKE ? OR item_category.cat_name LIKE ? AND 
          item_master.active=? 
          ORDER BY item_master.item_name ASC LIMIT 50 ';
      /*OR  item_master.active=? */
          $se = escape($this->getParams()[0]);
          $this->data['re'] = $this->_item->getFromQ($quary,array('%'.$se.'%','%'.$se.'%','%'.$se.'%','1'));
        }else{
        $quary =  'SELECT item_master.*, item_category.cat_name, user_master.name, user_master.service_no, user_master.user_rank 
        FROM item_master 
        LEFT JOIN user_master ON item_master.latest_user_tbl_id = user_master.user_id 
 ...
 public function view_bmd_iv(){
  ...
  if(isset($this->getParams()[0])){
         ...
         $query =   'SELECT bmd_iv_details.*,
                            ...
                            FROM
                            bmd_iv_details
                            LEFT JOIN item_master ON bmd_iv_details.item_code = item_master.item_code
                            LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
                            LEFT JOIN user_master ON bmd_iv_details.latest_user_tbl_id = user_master.user_id
                            WHERE
 ...
 public function crv_search(){
  ...
  if(isset($this->getParams()[0])){
   ...
   $quary =  'SELECT bmd_crv_header.*, crv_types.crv_type_txt, supplier_master.supp_name,user_master.name, user_master.service_no, user_master.user_rank FROM bmd_crv_header 
        LEFT JOIN crv_types ON bmd_crv_header.crv_type_id = crv_types.crv_type_id
        LEFT JOIN supplier_master ON bmd_crv_header.supplier_id = supplier_master.supp_id
         LEFT JOIN user_master ON bmd_crv_header.latest_user_tbl_id = user_master.user_id 
...
public function bmdiv_search(){
  ...
  if(isset($this->getParams()[0])){
   ...
   $quary =  'SELECT bmd_iv_header.*, user_master.name, user_master.service_no, user_master.user_rank, stations.station_name, item_category.cat_name FROM bmd_iv_header 
        LEFT JOIN stations ON bmd_iv_header.iv_to = stations.station_id
        LEFT JOIN item_category ON bmd_iv_header.iv_type = item_category.item_cat_id
         LEFT JOIN user_master ON bmd_iv_header.latest_user_tbl_id = user_master.user_id 
        WHERE  bmd_iv_header.iv_type!=? AND bmd_iv_header.active = ? AND bmd_iv_header.iv_no LIKE ?  ORDER BY bmd_iv_header.iv_header_id DESC LIMIT 20';

   $this->data['re'] = $this->_crv->getFromQ($quary,array('8','1','%'.escape($this->getParams()[0]).'%')); 
  }else{
    $quary = 'SELECT bmd_iv_header.*, user_master.name, user_master.service_no, user_master.user_rank, stations.station_name, item_category.cat_name FROM bmd_iv_header 
        LEFT JOIN stations ON bmd_iv_header.iv_to = stations.station_id
        LEFT JOIN item_category ON bmd_iv_header.iv_type = item_category.item_cat_id
         LEFT JOIN user_master ON bmd_iv_header.latest_user_tbl_id = user_master.user_id 
        WHERE  bmd_iv_header.iv_type!=? AND bmd_iv_header.active = ?  ORDER BY bmd_iv_header.iv_header_id DESC LIMIT 20';
   $this->data['re'] = $this->_crv->getFromQ($quary,array('8','1')); 
  }
   $result = array();
        foreach ($this->data['re'] as $key => $value) {
 ...
 public function bmdinstiv_search(){
       $this->_crv = new bmdivs();
  if(isset($this->getParams()[0])){
  ...
  public function bmdinstiv_search_layout(){
   return 'api';
 }
 ...
 public function item_stock_details(){
  ...
  if(isset($this->getParams()[0])){
   ...
   $dates = explode(' - ', $dates);
    ...
    $qname = 'SELECT item_master.item_name,master_unit.unit_name FROM item_master LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id WHERE  item_master.active = ? AND item_master.item_code = ?';
  ...
  $this->iv = new bmdivs();
     $dates = escape(input::get('date'));
                ...
                $query = 'SELECT bmd_iv_header.iv_to,
                          ...
                          FROM
                          bmd_iv_header
                          LEFT JOIN bmd_iv_details ON bmd_iv_header.iv_header_id = bmd_iv_details.iv_header_id
                          LEFT JOIN item_master ON bmd_iv_details.item_code = item_master.item_code
                          LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
        ...
        foreach ($this->data['re'] as $key => $value) {
          $query = 'SELECT   station_stock_master.qoh,
              ...
              if(count($temp)==1){  $result[]=array('item_code'=>$value->item_code,'item_name'=>$value->item_name,'unit'=>$value->unit_name,'qoh'=>$temp[0]->qoh,'rol'=>$temp[0]->rol);}else{
                 $result[]=array('item_code'=>$value->item_code,'item_bin_no'=>$value->item_bin_no,'item_name'=>$value->item_name,'unit'=>$value->unit_name,'qoh'=>'0','rol'=>'');
              }
    ...
    if(count($this->data['header'])>0){
           ...
           $query = 'SELECT  station_rv_details.*,
                             master_unit.unit_name,
                              ...
                              LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id WHERE station_rv_details.rv_header_id =? AND station_rv_details.active=?';
...
public function getexp_station(){
 ...
 public function view_iv(){
  ...
  $this->_iv = new ivs();
  $this->_station = new stations();
       /*  stations.station_name  LEFT JOIN stations ON  station_iv_header.iv_to=stations.station_id */
       ...
       $this->data['ivheader']->iv_to = $this->_station->getStation(array('station_id','=',$this->data['ivheader']->iv_to))[0]->station_name;
     $result = array();
  ...
  */
      ...
      $quary = 'SELECT  item_master.item_name,
                        ...
                        item_master
                        LEFT JOIN master_unit ON item_master.measure_unit_id = master_unit.measure_unit_id
                        LEFT JOIN station_stock_master ON item_master.item_code = station_stock_master.item_code
                        WHERE
 ...
 public function item_stockstation_layout(){
  ...
  if(isset($this->getParams()[0])){
   ...
   // $qiv = 'SELECT sum(bmd_iv_details.issue_qty) as iv FROM bmd_iv_details WHERE bmd_iv_details.active = ? AND bmd_iv_details.item_code = ? AND bmd_iv_details.system_date BETWEEN ? AND ?';
    ...
    $qiv = 'SELECT Sum(station_iv_details.issue_qty) as iv FROM station_iv_details LEFT JOIN station_iv_header ON station_iv_details.iv_header_id = station_iv_header.iv_id WHERE
    ...
    $qrv = 'SELECT Sum(station_rv_details.receive_qty) as rv FROM
            station_rv_details
            LEFT JOIN station_rv_header ON station_rv_details.rv_header_id = station_rv_header.rv_id
            WHERE
            station_rv_details.active = ? AND
