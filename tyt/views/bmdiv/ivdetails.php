                    ...
                    <div class="box-body">
                         ...
                         <form class="form-horizontal" method="post" enctype="multipart/form-data">
                          <div class="row">
                            ...
                            <input type="hidden" name="iv_header_id" value="<?= $data['ivheader'][0]->iv_header_id ?>"> 
                            ...
                            <div class="form-group">
                             ...
                             <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select  class="select3 form-control" name="batch_no" id="batch_no" >
                                      <option></option>
                                    </select>
                                    <sub>Batch number if not available type and press Enter</sub>
                                </div>
                            </div>  
                            ...
                            <div class="form-group">
                                ...
                                <!--  <input type="text" class="form-control" name="expire_date" id="expire_date" value="<?=input::get('expire_date')?>"> -->
                                    <select class="select3 form-control" id="expire_date"  name="expire_date"  >
                                    <option></option>
                                  </select> 
                                  <sub>Expire date if not available type and press Enter "YYYY-MM-DD"</sub>
                                  </div>
                                </div>
                            </div>
                        ...
                        $total= $i=0; foreach ($data['details'] as $key=>$value ) { $i++;
                         ?>
                          <tr>
                            ...
                            <td><?=($value->issue_status=='0')?'':$value->system_dt ?></td>
                            ...
                            <td  class="text-right">
                                  ...
                                  <?php if($value->issue_status=='0'){  ?>
                                    <a href="<?=HTTP_PATH.'bmdiv/ivdetails/'.$value->iv_header_id.'/?issue='.$value->bmd_iv_detail_id?>" class="btn btn-sm btn-success" data-toggle="tooltip" title="Issue">ISSUE</a>
                                      <?php if($data['admin']->privilage_id>1){  ?>
                                   <a href="<?=HTTP_PATH.'bmdiv/ivdetails/'.$value->iv_header_id.'/?del='.$value->bmd_iv_detail_id?>" class="btn btn-sm btn-danger"  onclick="return confirmDelete();" data-toggle="tooltip" title="Delete" ><i class="fa fa-trash"></i></a>   
                                <?php } }elseif($data['admin']->privilage_id>1){ ?>  
                                  <a href="<?=HTTP_PATH.'bmdiv/ivdetails/'.$value->iv_header_id.'/?undo='.$value->bmd_iv_detail_id?>" class="btn btn-sm btn-warning" data-toggle="tooltip" title="undo issue">UNDO</a>
                                <?php } ?>  
        ...
        <script>
           ...
           function loadItem(cat,select=null){
              ...
              $.each( result, function( key, val ) {
                  ...
                  $('#item_code').append('<option data-color="available" '+se+' data-qty="'+val['ablq']+'" data-unit="'+val['unit']+'" value="'+val['item_code']+'">'+val['item_name']+' ('+val['item_bin_no']+') '+' - '+val['qoh']+' '+val['unit']+'</option>');
              });
          ...
          function loadBadgeExp(value_select){
             $("#batch_no").select2('destroy');              
                ...
                $.ajax({url:"<?=HTTP_PATH.'ajax/getbadge/'?>"+value_select,
                success: function(result){
                  $.each( result, function( key, val ) {
                     $('#batch_no').append('<option value="'+val['batch_no']+'" >'+val['batch_no']+'</option>');
                   });
                  $('#batch_no').select2({  tags: true});
                }
                }); 
                ...
                $('#expire_date').children('option:not(:first)').remove();            
                 ...
                 $.ajax({url:"<?=HTTP_PATH.'ajax/getexp/'?>"+value_select,
                success: function(result){
                  $.each( result, function( key, val ) {
            $('#expire_date').append('<option value="'+val['expire_date']+'" >'+val['expire_date']+'</option>');
                   });
                  $('#expire_date').select2({  tags: true});
                }
                }); 
          }
