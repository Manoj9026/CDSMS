      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">All  IV Accessories</li>
          </ol>
        </section>
            ...
            <div class="row"> 
                <div class="col-xs-12">
                    ...
                    <div class="box-header">
                      ...
                      <div class="col-lg-4">
                          <a href="<?=HTTP_PATH?>bmdiv/instnewiv" class="btn btn-success btn-sm pull-right"><i class="fa fa-plus"></i> New</a>
                      </div>
                    </div> 
                    ...
                    <div class="box-body no-padding">
                        ...
                        <tr>
                            ...
                            <td class="text-right" >
                             ...
                             <a href="<?=HTTP_PATH.'bmdiv/printinstiv/'.$value->iv_header_id?>" target="_new" class="btn btn-sm btn-default" data-toggle="tooltip" title="Print IV" ><i class="fa fa-print"></i></a>
                            ...
                            <button  onclick="getContent(<?=$value->iv_header_id?>)" class="btn btn-sm btn-warning" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fa fa-eye"></i></button>
                              ...
                              <a href="<?=HTTP_PATH.'bmdiv/instivdetails/'.$value->iv_header_id?>" class="btn btn-sm btn-success" data-toggle="tooltip" title="Add IV details" ><i class="fa fa-bars"></i></a>
                                <?php if($data['admin']->privilage_id>1){  ?>
                                <a href="<?=HTTP_PATH.'bmdiv/instnewiv/'.$value->iv_header_id?>" class="btn btn-sm btn-info" data-toggle="tooltip" title="Edit" ><i class="fa fa-pencil"></i></a>
                                 <?php } ?>
                            </td>
                        </tr>
                        <?php  } ?></tbody>
...
<!-- model for show the details -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-xs-6">
                  <ul class="list-group">
                    <li class="list-group-item d-flex justify-content-between align-items-center">Reference <span id="crv_no" class="badge badge-primary badge-pill "></span></li>
 ...
 <script type="text/javascript">
   function getContent(iv){
              ...
              if(val['issue_status']=='1'){ status=val['system_dt'];}else{ status='<lable class="badge bg-yellow">Not Issued</lable>'}
               $('#model-table tbody').append('<tr><td>'+i+'</td><td>'+val['item_name']+'<td>'+val['item_bin_no']+'</td>'+'</td><td class="text-right">'+val['issue_qty']+' '+val['unit_name']+'</td><td>'+val['batch_no']+'</td><td>'+val['expire_date']+'</td><td>'+status+'</td></tr>');
                 ...
                 $('#model-table thead').hide();
                   $('#model-table tbody').append('<tr><th   class="text-center"> Iitems not added!.<br/><a href="<?=HTTP_PATH?>bmdiv/instivdetails/'+result['header']['iv_header_id']+'" class="btn bnt-sm btn-success"> <i class="fa fa-plus"></i> Add Items</a>  </th></tr>');
                }
              }});
          }
           ...
           if((search).length>2){
                $.ajax({url: "<?=HTTP_PATH.'ajax/bmdinstiv_search/'?>"+search,
                    success: function(result){ 
              ...
              $.each( result, function( key, val ) {
               ...
               $('#result-table tbody').append('<tr><td>'+i+'</td><td>'+val['iv_no']+cou+'</td><td>'+val['iv_date']+'</td><td>'+val['station_name']+'</td><td>'+val['cat_name']+'</td><td>'+val['authority']+'</td><td class="text-right" > <span class="btn btn-sm btn-info" data-html="true" data-toggle="tooltip"  title="<table   class=\'\' ><tr><td class=\'text-left\'>User</td><td class=\'text-center\'> : </td><td class=\'text-left\'> '+val['service_no']+' '+val['user_rank']+' '+val['name']+'</td></tr><tr><td class=\'text-left\'>IP</td><td class=\'text-center\'> : </td><td class=\'text-left\'> '+val['latest_ip']+'</td></tr><tr><td class=\'text-left\' >Date Time</td><td class=\'text-center\'> : </td><td class=\'text-left\'> '+val['system_dt']+'</td></tr></table>" > <i class="fa fa-info"></i></span> <a href="<?=HTTP_PATH.'bmdiv/printinstiv/'?>'+val['iv_header_id']+'" target="_new" class="btn btn-sm btn-default" data-toggle="tooltip" title="Print IV" ><i class="fa fa-print"></i></a> <button  onclick="getContent('+val['iv_header_id']+')" class="btn btn-sm btn-warning" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fa fa-eye"></i></button> <a href="<?=HTTP_PATH.'bmdiv/instivdetails/'?>'+val['iv_header_id']+'" class="btn btn-sm btn-success" data-toggle="tooltip" title="Add IV details" ><i class="fa fa-bars"></i></a> <a href="<?=HTTP_PATH.'bmdiv/instnewiv/'?>'+val['iv_header_id']+'" class="btn btn-sm btn-info" data-toggle="tooltip" title="Edit" ><i class="fa fa-pencil"></i></a></td></tr>');  i++;
                ...
                $.ajax({url: "<?=HTTP_PATH.'ajax/bmdinstiv_search/'?>",
                    success: function(result){ 
                      $('#result-table tbody').empty();                       
                      var i = 1; var btn ='';     
              $.each( result, function( key, val ) {
