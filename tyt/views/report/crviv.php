    ...
    <?php  include VIEWS_PATH.'/include/supper-header.php'; ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1> CRV and IV Details for a  item
            <small></small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="<?=HTTP_PATH?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Reports</li>
             <li class="active">CRV and IV Details for a Item</li>
          </ol>
        </section>
        ...
        <section class="content">
            ...
            <div class="row"> 
              ...
              <h3 class="box-title"><!--CRV and IV Details for a Items--></h3>
              ...
              <h3 class="box-title">
              	...
              	<label class="label label-warning">Re-Order level: <?=$data['item']->rol.' '.$data['item']->unit_name?></label></h3>
                    <div class="box-tools ">
                          <div class="col-lg-8">  
                        </div>
                    </div>
                    ...
                    <?php if($data['crv']){  ?>
            				<table class="table   table-hover ">
            					...
            					<?php $crvTotal =0; foreach ($data['crv'] as $key => $value) { $crvTotal+= $value->qty;?>
            						 <tr>
            						 	<td><?=$value->crv_no?></td>
            						 	<td><?=$value->crv_date?></td>
            						 	<td><?=$value->expire_date?></td>
            						 	<td class="text-right"><?=$value->qty.' '.$data['item']->unit_name?></td>
            						 	<td>
                  ...
                  <?php }else{ ?>
                    <div class="row text-center">No CRV Details</div>
                  <?php } ?>
            			</div>
            		</div>
                    ...
                    <?php if($data['iv']){ ?>
                        ...
                        <th class="text-right">Quantity</th>
            					...
            					<?php $ivTotal =$ivReseve = 0; foreach ($data['iv'] as $key => $value) { 
                                                   if($value->issue_status=='1'){ $ivTotal+=$value->issue_qty; }
                                                   else{
                                                     $ivReseve+=$value->issue_qty;  
                                                   }
                                                    
                                                    ?>
                                                        <tr class="<?= ($value->issue_status=='0')?'text-red':'';?>">
            						 	<td><?=$value->iv_no?></td>
                          ...
                          <td class="text-right  "><?=$value->issue_qty.' '.$data['item']->unit_name?></td>
            					...
            					<?php } ?>
                                                         <tr class="text-red">
                                                             <th colspan="2" class="text-right">Total Not Issued</th>
                                                             <th  class="text-right"><?=$ivReseve.' '.$data['item']->unit_name?></th>
                                                         </tr>
                      <tr>
