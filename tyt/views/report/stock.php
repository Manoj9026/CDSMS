        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Reports</li>
             <li class="active">Category wise stock</li>
          </ol>
        </section>
        ...
        <section class="content">
            ...
            <div class="row"> 
              ...
              <h3 class="box-title"><!--Curent stock  Report --></h3>
                    ...
                    <div class="box-body">
                     <form class="form-horizontal" method="post" enctype="multipart/form-data" action="<?=HTTP_PATH.'report/stock'?>">
                      <input type="hidden" value="<?=token::genarate()?>" name="token">
                         
                         <div class="form-group">
                                <label class="col-sm-3">Item Category <sup class="text-red">*</sup></label>
                                <div class="col-sm-9">
                                   <select name="item_cat_id[]" multiple="multiple" id="item_cat_id" class="form-control select2">
                                    <option></option>
                                       
                                       <?php foreach ($data['category'] as $key => $value) { 
                                        ?>
                                       <option value="<?=$value->item_cat_id?>" <?= (input::get('item_cat_id')!='' AND in_array($value->item_cat_id, input::get('item_cat_id')))?'selected="true"':''; ?> ><?=$value->cat_name?></option>
                                       <?php } ?>
                                   </select>
                                  </div>
                                </div>
                                ...
                                <div class="col-sm-9">
                                   <select name="station_id"   id="station_id" class="form-control select2">
                                    <option></option>
                                       ...
                                       <option value="<?=$value->station_id?>" <?= ($value->station_id==input::get('station_id'))?'selected="true"':''; ?> ><?=$value->station_name?></option>
                                       <?php } ?>
                                   </select>
                                  </div>
                                </div>
               ...
               // print_r($data['result']) ?>
                <div class="box result box-lime">
                  <div class="box-header">
                    ...
                    <div class="box-tools ">
                      <a href="<?=HTTP_PATH.'report/pdf_stock/?station_id='.base64_encode(input::get('station_id')).'&&type='.input::get('type').'&&'.$data['other']?>" target="_new" class="btn btn-sm btn-danger" data-toggle="tooltip" title="Download PDF as PDF"><i class="fa fa-file-pdf-o"></i></a>
                     <!-- <?=$data['pagination']?> -->
                      </div>
                  </div>
                  ...
                  <div class="box-body no-padding ">                    
                   <table class="table table-hover">
                    <thead>
                     <tr>
                       <th width="5%">No</th>
                       <th width="85%">Item Name <i class="fa fa-sort-alpha-asc text-lime"></i> <span class="pull-right">Bin No</span></th>
