...
class db{
    ...
    private $_pdo,
            ...
            $_where,
            $_order,
            $_operator = array('=','>','<','>=','<=','IN','NOT IN','!=','LIKE');
    ...
    public function __construct() {
        try { 
         //   print_r(config::get('mysql/password'));
            $this->_pdo = new PDO('mysql:host='.config::get('mysql/host').';dbname='.config::get('mysql/db'), config::get('mysql/username'), config::get('mysql/password'),array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET sql_mode=""'));
        ...
        */ 
         ...
         * public get function select query create and process using action function
         */
        public function getSelect($table,$fields,$where=NULL,$order=NULL){  
          $select = 'SELECT  ';  foreach ($fields as $k=>$v){ $select.=$v.','; }
            return $this->action(trim($select,','), $table,$where,$order);
        }
